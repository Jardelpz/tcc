; fazer com presidente getulio/ilhota
; 5 escolas municipais, 2 estaduais
; pegar a qtd de pessoas por bairro
; capacidade da escola
; monitor em tempo real
; realizar simulacoes por tipo: pessoas saindo da escola municipal para estaduais, primeira escolha de escola
; usar taxa de crescimento populacional (2,8% ao ano) por bairro
; estressar a superlotacao, onde criar a nova escola



; criar logica para capacidade das escola - ok
; gerar metricas dos resultados - ok
; variavel de tempo, a cada ano uma nova escolha (o que for melhor), - ok
; capactidade por nivel, basico, fundamental e medio (capacidade da escola)
;fazer com que um ano ja calculado nao volte a recalcular salario - ok

; adicionar bairro menos rico?
extensions [gis]

globals [victor-konder itoupava-seca density-victor-konder density-itoupava-seca area-victor-konder area-itoupava-seca scale-factor year-state]

breed [schools a-school]
breed [students a-student]

schools-own [name school-type area-name available-class capacity]
students-own [chosen-school start-age age income area-name class]
...
to go
  ifelse year-state = -1 and years = 0[
    ...
    ifelse years > year-state [ ;nao deixa repetir a mesma escolha no ano
      ...
      print (word "nao é possivel voltar no tempo")
    ]
  ]
  tick
end
...
to show-results
  ...
  output-print (word "Média de renda Victor Konder: " average-income-per-area "VICTOR KONDER")
  ...
  foreach school-list [
    school ->
    output-print (word [name] of school ": " students-per-school school)
  ]
...
to-report redistribute-income [income-to-update]
  let number random 100
  if number < 50 [  ;; 50% de chance de aumento
    let increment (random 301 + 100)
    report income-to-update + increment
  ]
  ifelse number < 90 [
    let increment (random 901 + 100)
    report income-to-update + increment
  ][
    let increment (random 3001 + 1000)
    report income-to-update + increment
  ]
end
...
to-report average-income-per-area [area]
  ...
  if any? students-in-area [
    report mean [income] of students-in-area
  ]
  report 0
end


to-report students-per-school [school]
