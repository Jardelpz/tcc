; fazer com presidente getulio/ilhota - ok
; 5 escolas municipais, 2 estaduais - ok
; pegar a qtd de pessoas por bairro - ok
; criar shape file de cada bairro (15) - ok
; adicionar info de densidade e area em km² no shapefile - ok
; usar taxa de crescimento populacional (2,8% ao ano) por bairro - ok
; capacidade da escola - falta dado real
; verificar se a renda ta sendo atualizda com o passar dos anos e nao so uma vez - todo
; adicionar goOnce e golive - todo
; fazer com que o botao go avance um ano - todo

; monitor em tempo real - ok
; realizar simulacoes por tipo: pessoas saindo da escola municipal para estaduais, primeira escolha de escola - todo
; quantidade de alunos de cada escola - ok

; estressar a superlotacao, onde criar a nova escola - todo
; criar animacao de estudantes indo para escolas

; slider começar com a densaidade pre-definida porem permitindo aumentar - ok
; ao identidficar um aluno formado, ou matar ou remover a refernencia da escola - ok
; na redistribuicao de renda usar inflacao e 10% ter algum aumento de salario - ok
; quando ha superlotacao, exe 10 vagas preenchidas, um aluno foi para a escola publica, agora a escola tem 9, mas na execucao ninguem escolhei ela (estranho pois teria muitos estudantes que optariam por ela) - ok
; pq na lotacao total apos 2 execucao ta ficando 3 com lotacao maxima  sendo que so deveria ter 1, pois 5 estao formados e 1 esta sem escola - ok
; ao nao setar chosen-school como nobody no leave-school nao esta calculando certo o numero de estudantes com a escola - ok
; priorizar estudantes que nao tem escola na escolha primeiro - ok
; logo apos criar novos estudantes ja envia para escolha de escola - ok
; entender pq nas escolas ta ficando com x e vermelho o agente - quando um aluno em alguma execucao nao tinha escola, depois conseguiu) - ok
; rever students-moved-between-areas, ta retornando info falsa - com a alteracao do shapefile, o nome do bairro estava diferente) - ok

; quero gerar grafico da quantidade de vagas restante por escola - ok
; quero gerar grafico da quantidade de alunos por bairro - ok
; renda media dos alunos - ok
; quantidade de alunos formados (> 18) - ok
; quantidade de pessoas que estudavam numa municipal e foram para estadual
; quantidade de alunos que tiveram que sair dos seus bairros para estudar - ok
; primeiro escolha de escola
...
globals [ilhota ab bla ba bb bc bs bv bdb ce il mi mis pda poc sj year-state]

breed [schools a-school]
breed [students a-student]

schools-own [name school-type area-name available-class capacity]
...
to go
  ifelse year-state = -1 and years = 0 [
    ask students [
    choose-school
  ]
    set year-state 0
  ][
    ifelse years > year-state [ ;nao deixa repetir a mesma escolha no ano
      ask students [
        let ag start-age + years
        if ag > 18 [
          leave-school
        ]
      ]
      ...
      foreach sorted-students [ student ->
        ask student [
          let ag start-age + years
          set age ag
      ...
      print (word "Its not possible going back in time")
    ]
  ]
  tick
end
...
;    print(word "----------------")
;    print(word "district " district)
    let current-num-students students-per-area district

;    print(word "current students " current-num-students)
...
to-report school-accept-student [school]
  ...
  if [count students-here] of school < [capacity] of school [
...
"EEB Valério Gomes" 1.0 0 -2674135 true "" "plot students-per-school \"EEB Valério Gomes\""
